...
public class ParkingProblem {
    ...
    public ParkingProblem() {
        parking = parking.buildLinkedList(); // build parking place : Double Cycle Linked List
        currentNode = parking.getHead().getNext(); // next to the head node
    ...
    public static int calcCars() {
        while (flag) { // main loop begin
            if (currentNode.getData() != oldSign) { // usual node : no sign ‘V’
